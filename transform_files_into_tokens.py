...
def process_files(path_file, name_file):
    ...
    df_spectial_token_24h = mark_special_tokens(df, special_tokens=special_tokens, hour_toks=hour_context_size_24h, data_freq=frequency_mapping[name_file])
    df_spectial_token_12h = mark_special_tokens(df, special_tokens=special_tokens, hour_toks=hour_context_size_12h, data_freq=frequency_mapping[name_file])
    ...
    for column in list_columns:
        ...
        # Save scalers for this column (sanitize column name for filename)
        safe_column_name = sanitize_filename(column)
        joblib.dump(scaler_standard, f"scalers/{name_file}_column_{safe_column_name}_standard.pkl")

        # Define processing configurations: (data, special_token_series, norm_type, ebos_suffix)
        # special_token_series is used to identify WHERE special tokens should be inserted
        configs = [
            (y_standard_scaled, y_value_feature_st_12h, 'standard', '12h'),
            (y_standard_scaled, y_value_feature_st_24h, 'standard', '24h'),
        ...
        for data, special_token_series, norm_type, ebos_suffix in configs:
            ...
            tokenized_dfs[f"{norm_type}_simp_{ebos_suffix}_disc"][column] = pd.Series(y_simple_tok)
            ...
            if USE_BPE:
                ...
                tokenized_dfs[f"{norm_type}_simp_{ebos_suffix}_bpe"][column] = pd.Series(y_tokens_bpe)
            ...
            #     tokenized_dfs[f"{norm_type}_adapt_{ebos_suffix}_bpe"][column] = pd.Series(y_tokens_bpe)
...
def encode_token(data_list, base_name):
    """
    Encode pre-discretized token sequence using TokenBasedTokenizer.
    
    Parameters:
    -----------
    data_list : list or array of integers
        Pre-discretized tokens from simple_discretize or adaptative_bins_discretize.
        Special tokens like <EBOS> and <PAD> are already converted to their numeric IDs.
    base_name : str
        Base name for model and vocab files
    ...
    # Use TokenBasedTokenizer for pre-discretized token sequences
    if not joblib.os.path.isfile(model_name):
        ...
        objtok.train(data_list, total_vocab_size, verbose=True)
        objtok.save(fr"{base_name}_COM_BPE", file_vocab_name)
    else:
        objtok = TokenBasedTokenizer(N_samples, file_vocab_name, special_tokens=special_tokens)
        objtok.load(model_name)
    
    encoded_ids = objtok.encode(data_list)
    return encoded_ids

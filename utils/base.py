...
def render_token(t, encoding_name) -> str:
    # pretty print a token, escaping control characters
    s, n_edges = decode_with_float_vocab(t, ).decode('utf-8', errors='replace')
...
class Tokenizer:
    ...
    def search_recursively_tokens(self, token) -> list:
        if token > self.actual_vocab_size:
            for (p0, p1), idx in self.merges.items():
                if idx == token:
                    return self.search_recursively_tokens(p0) + self.search_recursively_tokens(p1)
        else:
            return [token]
        return [token]